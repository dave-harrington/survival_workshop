...
### Using $R$-year survival probabilities with an exponential distribution

Suppose the $R$-year survival probability is $S_1(R)$ in group 1 and $S_0(R)$ in group 0.  
...
Hence, doubling the hazard rate from group 1 to group 0 corresponds to doubling the log of the $R$-year survival probability.  
...
Suppose the 5-year survival probability on treatment A is 20\% and investigators want 90\% power to detect an improvement to 30\%.  
...
From the earlier calculations, the trial needs at least 215 events to have 80% power.

Try an initial guess of 250 patients enrolled over 5 years, and followed for 1 year.

The parameters:

- \texttt{tref} is 9 months, or 0.75 years, and since that is the median in the control group, \texttt{mc} = 0.5 = $S_c(0.75)$ 
- \texttt{n} = 250 patients enrolled over 3 years 
- \texttt{accrual} = 5, and set \texttt{tmin} = 1 year of follow-up
- \texttt{alpha} = 0.05   
...
library(Hmisc)
cpower(tref = 0.75, n = 250, mc = 0.5, 
       r = 28, accrual = 5, tmin = 1, 
...
- 89\% power because of 114 + 103 = 217 events

Perhaps the study can enroll fewer patients and follow longer

Examine slightly lower enrollment: 220 patients over 5 years, varying lengths of follow-up time
...
for(jj in 1:num.followup.periods){
  p[1,jj]= cpower(tref = 0.75, n = 220, mc = .50, r = perc.reduct, 
...
220 patients would have to be followed for 4 years to get the same power.
...
```{r eval=FALSE, echo=TRUE}
accrual.period = c(5,6)
enrollment.total = 44*accrual.period
...
```{r eval=TRUE, echo=TRUE}
colnames(power.table) <- c("yrs follow-up", rep("-", 4))
rownames(power.table) <- c("yrs enrollment", rep("-", 2))
round(power.table, digits = 3)
```

### Several alternative designs

- Enroll for 5 years, follow for 5 years and follow for 5 years.

- Enroll for 6 years and follow for 1 year.

Better design depends on the cost of enrollment vs follow-up.
...
- What happens to power with 20\% non-adherence on the intervention arm, even at the higher enrollment rate of 50 patients per year.
